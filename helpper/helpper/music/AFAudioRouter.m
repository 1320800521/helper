...
void onAudioRouteChange (void *clientData, AudioSessionPropertyID inID, UInt32 dataSize, const void* inData) {
    ...
    if( IS_DEBUGGING == YES ) {
//        DLog(@"==== Audio Harware Status ====");
//        DLog(@"Current Input:  %@", [AFAudioRouter getAudioSessionInput]);
//        DLog(@"Current Output: %@", [AFAudioRouter getAudioSessionOutput]);
//        DLog(@"Current hardware route: %@", [AFAudioRouter getAudioSessionRoute]);
//        DLog(@"==============================");
    }
    
    if( IS_DEBUGGING_EXTRA_INFO == YES ) {
//        DLog(@"==== Audio Harware Status (EXTENDED) ====");
...
//        DLog(@"Audio old route: %@", oldRoute);
//        DLog(@"Audio new route: %@", newRoute);
//        DLog(@"=========================================");
    }
}
...
+(NSString *)getAudioSessionRoute {
    ...
    if (route == NULL) {
//        DLog(@"Silent switch is currently on");
        return @"None";
    }
